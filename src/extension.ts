...
import { INCREMENT_COUNT_NOTIFICATION, SERVICE_NAME, INCREMENT_COUNT_COMMAND, GET_COUNT_REQUEST } from './constants';
...
export async function activate(context: ExtensionContext) {
	...
	context.subscriptions.push(commands.registerCommand(INCREMENT_COUNT_COMMAND, () => {
		store.increment();
	...
	vsls.onDidChangeSession(async (e) => {
		if (e.session.role === Role.Host) {
			...
			service!.onRequest(GET_COUNT_REQUEST, () => { return store.count; });
